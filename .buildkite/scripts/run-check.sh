...
if (($# != 0)); then
  printf 'Buildkite validation failed: positional arguments are forbidden\n' >&2
  exit 1
fi

die() {
  printf 'Buildkite validation failed: %s\n' "$*" >&2
  exit 1
}
...
case "$check" in
  ...
  shell)
    ...
    shell_paths=(
      ...
      install-one-line.sh
      test-matrix.sh
      run-full-test-matrix.sh
      tools/install-hostpanel-build.sh
      tools/validate-production-vm.sh
      tools/run-qemu-vm-acceptance.sh
      tools/qemu-guest-install.sh
      tools/install-update-agent.sh
      /tmp/install.generated.sh
    )
  ...
  metadata)
    ...
    if [[ -f RELEASE-MANIFEST.json && -f tools/validate_release_manifest.py ]]; then
      ...
      if [[ -f tests/test_release_manifest.py ]]; then
        python3 -m unittest -v tests.test_release_manifest
      fi
    else
    ...
    if [[ -n "${BUILDKITE_TAG:-}" ]]; then
      python3 tools/release_legal.py --repository-root .
    else
      printf 'Legal publication gate is enforced only for tagged builds.\n'
    fi
    ;;
  ...
  qemu-contracts)
    ...
    python3 -m py_compile \
      ...
      tools/sanitize-qemu-evidence.py \
      tools/seal-qemu-evidence.py \
      tools/build-update-release.py \
      tools/hostpanel-update.py
    for test_file in \
      test_qemu_vm_acceptance.py \
      test_qemu_token_scope.py \
      test_qemu_evidence_sanitizer.py \
      test_qemu_early_evidence.py \
      test_qemu_evidence_sealer.py \
      test_qemu_default_version.py \
      test_qemu_reboot_wait.py \
      test_systemd_resolved_dns.py \
      test_post_install_health.py \
      test_first_reboot_hardening.py
    do
  ...
  *)
    die "unknown or missing HP_BUILDKITE_CHECK: ${check:-<empty>}"
    ;;
esac
