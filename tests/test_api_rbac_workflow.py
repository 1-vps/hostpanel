...
WORKFLOW = ROOT / ".github" / "workflows" / "api-rbac-foundation.yml"
DOCUMENTATION = ROOT / "API-RBAC-FOUNDATION.md"


class ApiRbacWorkflowTests(unittest.TestCase):
    @classmethod
    ...
    def test_workflow_runs_complete_rbac_contract(self) -> None:
        ...
        self.assertIn("tests.test_hostpanel_api_rbac_impersonation", self.source)
        self.assertIn("tests.test_hostpanel_api_rbac_security", self.source)
        self.assertIn("tests.test_api_rbac_workflow", self.source)
    ...
    def test_path_filters_cover_every_contract_input(self) -> None:
        for path in (
            ...
            "tests/test_api_rbac_workflow.py",
        ):
            self.assertGreaterEqual(self.source.count(f"- {path}"), 2, path)

    def test_documentation_keeps_security_and_delivery_boundaries_explicit(self) -> None:
        for phrase in (
            "does not expose HTTP routes",
            "Explicit deny always wins",
            "intersection of the actor's and target's current capability",
            "call `authorize_impersonation()`",
            "Not yet delivered",
