...
test('quick navigation follows CSS-hidden navigation ancestors', async ({ page }) => {
  await signIn(page);
  ...
  await page.addStyleTag({
    content: '.hp-permission-parent-hidden { display: none !important; }',
  });
  await navLink.evaluate((link) => {
    link.parentElement.classList.add('hp-permission-parent-hidden');
  });
  await expect(navLink).toBeHidden();
  await expectControlsHidden(controls, true);

  await navLink.evaluate((link) => {
    link.parentElement.classList.remove('hp-permission-parent-hidden');
  });
  await expect(navLink).toBeVisible();
  await expectControlsHidden(controls, false);
});
