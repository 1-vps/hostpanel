...
run passes. Do not mark a pull request ready or merge based on static inspection.
...
workspaces, test-result uploads, artifacts, SSH reruns, or fork PR builds. The only
CircleCI-managed credential is the read-only checkout credential installed by the
GitHub App integration. The trusted config removes it immediately after checkout
and before repository commands execute.

## Worker images

CI workers require Python 3, PyYAML, Bash, Git, OpenSSL, ShellCheck, Docker, GNU
coreutils `timeout`, and the packages required by `test-matrix.sh`. QEMU workers
also require KVM, QEMU, `setfacl`, and the production-validator dependencies.
...
context, project variable, cache, workspace, artifact, or command argument.
...
7. Destroy every worker and retain the external runner logs plus CircleCI pipeline
   and job identifiers.
