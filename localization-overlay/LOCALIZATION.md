...
Machine assistance may be used to establish complete key parity, but a catalog is not promoted solely because it contains every key. Login, authentication, destructive actions, billing, security, compliance, backup, restore, migration, and provider operations require an additional terminology pass. Japanese, Portuguese, and Simplified Chinese remain release candidates until native-speaking subject-matter review has covered legal, billing, tax, contractual, and jurisdiction-specific language. See `LOCALIZATION-REVIEW-v3.4.0-overlay.md` for this expansion's review record and acceptance boundary.
...
6. Run both localization audits and the complete UI/backend test suite.
