...
def validate_reviewed_payload(label: str, payload: dict[str, dict[str, str]], expected_counts: dict[str, int], expected_digest: str) -> None:
    if set(payload) != set(expected_counts):
        raise SystemExit(f"{label} locale mismatch: expected {sorted(expected_counts)}, got {sorted(payload)}")
    ...
    if digest != expected_digest:
        raise SystemExit(f"{label} digest mismatch: expected {expected_digest}, got {digest}")
...
def load_review_files(overlay: pathlib.Path, file_names: tuple[str, ...], pattern: str, allowed_locales: frozenset[str], label: str) -> dict[str, dict[str, str]]:
...
def install_final_override_loader(core) -> None:
    ...
    def load_override_bundle(overlay: pathlib.Path, overrides: dict[str, dict[str, str]]) -> None:
        ...
        validate_reviewed_payload("base reviewed override", base_payload, EXPECTED_BASE_COUNTS, EXPECTED_BASE_CANONICAL_SHA256)
        original(overlay, overrides)

        final_payload = load_review_files(
            overlay, FINAL_OVERRIDE_FILES, "catalog-final-overrides.*.json",
            frozenset(core.RELEASE_CANDIDATES), "final semantic override"
        )
        validate_reviewed_payload("final semantic override", final_payload, EXPECTED_FINAL_COUNTS, EXPECTED_FINAL_CANONICAL_SHA256)
        ...
        for locale, entries in final_payload.items():
            ...
            if overlap:
                raise SystemExit(f"reviewed override layers overlap for {locale}: {overlap[:8]}")
            visible_payload[locale].update(entries)
        validate_reviewed_payload("combined source-visible override", visible_payload, EXPECTED_VISIBLE_COUNTS, EXPECTED_VISIBLE_CANONICAL_SHA256)

        ui_payload = load_review_files(
            overlay, PORTUGUESE_UI_OVERRIDE_FILES, "catalog-visible-ui-overrides.*.json",
            frozenset({"pt"}), "Portuguese UI override"
        )
        validate_reviewed_payload("Portuguese UI override", ui_payload, EXPECTED_UI_COUNTS, EXPECTED_UI_CANONICAL_SHA256)
        ui_overlap = sorted(set(visible_payload["pt"]) & set(ui_payload["pt"]))
        if ui_overlap:
            raise SystemExit(f"Portuguese UI override overlaps reviewed values: {ui_overlap[:8]}")
