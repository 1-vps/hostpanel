#!/usr/bin/env python3
"""Hardened loader for the Varnish runtime patcher."""
...
_IMPL_PATH = pathlib.Path(__file__).with_name('patch_varnish_runtime_impl.py')
_SPEC = importlib.util.spec_from_file_location(
    '_hostpanel_patch_varnish_runtime_impl', _IMPL_PATH
)
if _SPEC is None or _SPEC.loader is None:
    raise SystemExit(f'cannot load Varnish runtime patcher: {_IMPL_PATH}')
...
def _open_temporary(path: pathlib.Path, mode: int) -> tuple[int, pathlib.Path]:
    ...
    for _ in range(32):
        temporary = path.with_name(
            f'.{path.name}.varnish.{secrets.token_hex(12)}'
        )
        try:
